...
- 规则引擎：四柱、五行、十神、格局、喜用神、大运和流年等确定性计算。
- RAG 引用：来自本地知识库的片段，响应中保留来源、检索路由、相似度和评分。
- LLM 解读：基于规则结果和检索上下文生成自然语言说明。

报告响应包含 `traceability` 字段，用于区分哪些内容来自引擎、知识库或模型综合。项目也提供黄金样例评测，便于检查节气边界、真太阳时、时区、闰月和大运顺逆等关键命理结果。

本项目定位为文化娱乐与自我反思工具，不构成医疗、法律、投资或人生重大决策建议。
...
```powershell
.\.venv\Scripts\python.exe scripts\evaluate_bazi_goldens.py
```
