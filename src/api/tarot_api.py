...
from src.api.streaming import (
    chunk_text,
    iter_langgraph_updates,
    merge_langgraph_update,
    sse_event,
    sse_response,
)
...
def _prepare_tarot_analysis(input_data: TarotInput):
    ...
    return request_payload, session_ctx, session, initial_state


def _finalize_tarot_analysis(
    *,
    ...
    response_text = (
        ...
        or normalized_output.get("synthesis")
        or safety_info.get("message", "占卜完成")
    )
    if session:
...
def _iter_tarot_progress_events(node_name: str, update: Dict[str, Any], yielded: set[str]):
    if node_name == "tool_node":
        if update.get("spread_info") and "spread" not in yielded:
            yielded.add("spread")
            yield {"node": node_name, "label": "选择牌阵"}
        if update.get("drawn_cards") and "draw" not in yielded:
            yielded.add("draw")
            yield {"node": node_name, "label": "抽牌"}

        executor_state = update.get("executor_state", {})
        if executor_state.get("card_interpretations") and "interpret" not in yielded:
            yielded.add("interpret")
            yield {"node": node_name, "label": "单牌解读"}

    if node_name == "agent_node" and update.get("llm_response") and "synthesis" not in yielded:
        yielded.add("synthesis")
        yield {"node": node_name, "label": "综合解读"}
...
async def analyze_tarot(input_data: TarotInput) -> TarotResponse:
    ...
    try:
        request_payload, session_ctx, session, initial_state = _prepare_tarot_analysis(input_data)
        ...
        _, response_data = _finalize_tarot_analysis(
...
async def analyze_tarot_stream(input_data: TarotInput):
    logger.info(f"收到塔罗流式占卜请求：{input_data.model_dump()}")

    async def event_stream():
        try:
            request_payload, session_ctx, session, initial_state = _prepare_tarot_analysis(input_data)
            final_state = dict(initial_state)
            yielded_steps: set[str] = set()
            ...
            async for chunk in tarot_app.astream(initial_state, stream_mode="updates"):
                for node_name, update in iter_langgraph_updates(chunk):
                    merge_langgraph_update(final_state, update)
                    for progress_event in _iter_tarot_progress_events(node_name, update, yielded_steps):
                        yield sse_event("progress", progress_event)

            if final_state.get("error") and final_state.get("status") != "safety_blocked":
                logger.error(f"塔罗流式工作流执行失败：{final_state['error']}")
                raise HTTPException(status_code=400, detail=final_state["error"])

            response_text, response_data = _finalize_tarot_analysis(
                request_payload=request_payload,
                session_ctx=session_ctx,
                session=session,
                final_state=final_state,
            ...
            for text_chunk in chunk_text(response_text):
                yield sse_event("partial", {"text": text_chunk})
                await asyncio.sleep(0)

            yield sse_event("result", {"data": response_data})
            yield sse_event("done", {"success": True})
        except HTTPException as exc:
            message = exc.detail if isinstance(exc.detail, str) else "请求处理失败"
            yield sse_event("error", {"message": message})
        except Exception as e:
            logger.error(f"流式塔罗占卜失败：{e}", exc_info=True)
            yield sse_event("error", {"message": f"服务器内部错误：{str(e)}"})
...
async def followup_tarot(input_data: TarotFollowupInput) -> TarotResponse:
    ...
    input_result = safety_checker.check_scene_input(input_data.query, SceneType.FOLLOW_UP)
    if input_result.blocked:
        return TarotResponse(
            ...
            data={
                ...
                "response": input_result.message,
                "safety": input_result.to_dict(),
            }
        )
    ...
    output_result = safety_checker.check_scene_output(response_text, SceneType.FOLLOW_UP)
    if output_result.blocked:
        response_text = safety_checker.get_safe_response(output_result.category, SafetyLevel.BLOCK)
    elif output_result.level == SafetyLevel.WARNING:
        logger.warning(f"⚠️ 塔罗追问输出有风险: {output_result.matched_keywords}")
    ...
    return TarotResponse(
        ...
        data={
            ...
            "safety": output_result.to_dict(),
        }
    )


@router.post("/followup/stream")
async def followup_tarot_stream(input_data: TarotFollowupInput):
    logger.info(f"收到塔罗流式追问请求：conversation_id={input_data.conversation_id}")

    async def event_stream():
        try:
            input_result = safety_checker.check_scene_input(input_data.query, SceneType.FOLLOW_UP)
            if input_result.blocked:
                response_text = input_result.message
                response_data = {
                    "conversation_id": input_data.conversation_id,
                    "response": response_text,
                    "safety": input_result.to_dict(),
            ...
            logger.error(f"流式塔罗追问失败：{e}", exc_info=True)
            yield sse_event("error", {"message": f"服务器内部错误：{str(e)}"})
