...
功能说明：
    本模块实现了八字格局的判断逻辑，基于《子平真诠》《渊海子平》等传统
    命理典籍的规则。格局是八字分析的核心概念，用于判断命局的特殊性质
    和命运特征。

格局分类：
    1. 从格（特殊格局）：日主极旺或极弱，宜顺其势
    2. 杂格（特殊格局）：魁罡格、日德格、日贵格等
    3. 正格（八正格）：正官格、七杀格、正印格、偏印格、正财格、偏财格、
       食神格、伤官格
...
class GejuEngine:
    ...
    功能说明：
        格局判断引擎，负责根据八字四柱和日主强弱判断命局的格局类型。
        格局是八字分析的核心概念，用于判断命局的特殊性质和命运特征。
    ...
    # 十神映射（日干为基准）
    # 十神表示其他干支与日干的关系
    # 比肩/劫财：同五行，阳阳/阴阴为比肩，阴阳为劫财
    # 食神/伤官：我生，阳阳/阴阴为食神，阴阳为伤官
    # 正财/偏财：我克，阳阳/阴阴为正财，阴阳为偏财
    # 正官/七杀：克我，阳阳/阴阴为正官，阴阳为七杀
    # 正印/偏印：生我，阳阳/阴阴为正印，阴阳为偏印
    SHISHEN_MAP = {
    ...
    def _check_congge(self, pillars: FourPillars, day_master_strength: Dict) -> Dict:
        ...
        返回值：
            Dict: 从格判断结果，包含：
                - is_cong (bool): 是否为从格
                - geju_type (str): 从格类型
                - description (str): 从格描述
                - strength (str): 格局强弱等级
        
        从格判断标准：
            1. 从强格：日主极旺（strength == 'very_strong' 或 score >= 80）
            2. 从弱格：日主极弱（strength == 'very_weak' 或 score <= 20）
    ...
    def _check_zage(self, pillars: FourPillars) -> Dict:
        ...
        返回值：
            Dict: 杂格判断结果，包含：
                - is_zage (bool): 是否为杂格
                - geju_type (str): 杂格类型
                - description (str): 杂格描述
                - strength (str): 格局强弱等级
        
        常见杂格：
            1. 魁罡格：壬辰、庚戌、戊戌、庚辰
            2. 日德格：甲寅、丙辰、戊辰、庚戌、壬辰
            3. 日贵格：丁未、丁丑、戊子、戊寅等
    ...
    def _check_zhengge(self, pillars: FourPillars, ri_qian: str, month_dizhi: str) -> Dict:
        ...
        功能说明：
            正格是八字格局中最常见的类型，根据月令本气的十神来判断。
            八正格包括：正官格、七杀格、正印格、偏印格、正财格、偏财格、
            食神格、伤官格。
        ...
        返回值：
            Dict: 正格判断结果，包含：
                - is_zage (bool): 是否为杂格（固定为 False）
                - geju_type (str): 格局类型
                - description (str): 格局描述
                - strength (str): 格局强弱等级
                - month_shishen (str): 月令本气十神
                - month_tg_shishen (str): 月干十神
